...
try {
    switch ($action) {
        case 'save_transaction':
            ...
            // Fix for the rounding issue - preserve exact user input for amount
            ...
            if (empty($amount) || !is_numeric($amount) || floatval($amount) <= 0) $missingFields[] = 'Amount';
            ...
            $mappedCategoryName = $categoryMappings[$budgetHeading] ?? $budgetHeading;
            ...
            $quarterBudgetQuery = "SELECT id, period_name, budget, actual, forecast, variance_percentage 
                                 FROM budget_data 
            ...
            // Set budget tracking values for budget_preview table
            $quarterPeriod = $quarterBudgetData['period_name'] ?? 'Unknown';
            $originalBudget = $quarterBudgetData['budget'] ?? 0; // Original planned budget (unchanged)
            $currentActual = $quarterBudgetData['actual'] ?? 0; // Current actual spending
            $actualSpent = $currentActual + $amount; // What actual will be after this transaction
            ...
            // Variance calculation: ((Budget - (Actual + Forecast)) / Budget) * 100
            ...
            // Add cluster column to the INSERT statement
            $stmt = $conn->prepare("INSERT INTO budget_preview (BudgetHeading, Outcome, Activity, BudgetLine, Description, Partner, EntryDate, Amount, PVNumber, DocumentPaths, DocumentTypes, OriginalNames, QuarterPeriod, CategoryName, OriginalBudget, RemainingBudget, ActualSpent, ForecastAmount, VariancePercentage, cluster, budget_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            // Add cluster parameter and budget_id to the bind_param call
            $stmt->bind_param("sssssssdssssssssddssi", $budgetHeading, $outcome, $activity, $budgetLine, $description, $partner, $entryDate, $amount, $pvNumber, $documentPathsStr, $documentTypesStr, $originalNamesStr, $quarterPeriod, $mappedCategoryName, $originalBudget, $remainingBudget, $actualSpent, $forecastAmount, $variancePercentage, $userCluster, $budgetId);
            ...
            if ($stmt->execute()) {
                ...
                // Update the budget_preview table with the budget_id for proper linking if not already set
        ...
        case 'check_budget':
            ...
            // Map budget heading to category name
            $categoryMappings = [
                'Administrative costs' => '1. Administrative costs',
                'Operational support costs' => '2. Operational support costs',
                'Consortium Activities' => '3. Consortium Activities',
            ...
            $categoryName = $categoryMappings[$budgetHeading] ?? $budgetHeading;
            
            // Find the correct quarter based on date ranges in database with cluster consideration
            $quarterQuery = "SELECT period_name, budget, actual, forecast, start_date, end_date 
                           FROM budget_data 
                           WHERE year2 = ? AND category_name = ? 
                           AND period_name IN ('Q1', 'Q2', 'Q3', 'Q4')
                           AND ? BETWEEN start_date AND end_date";
        ...
        case 'export_transactions_csv':
            ...
            fputcsv($fp, ['Budget Heading', 'Outcome', 'Activity', 'Budget Line', 'Transaction Description', 'Partner', 'Payment Date (dd/mm/yyyy)', 'Amount']);
        ...
        case 'export_combined_excel':
            ...
            try {
                ...
                // Grand Total Variance: (budget - (actual+forecast)) / budget * 100
                $gtVariance = 0;
                if ($grandTotal['annual_budget'] > 0) {
                    $gtVariance = (($grandTotal['annual_budget'] - $grandTotal['annual_actual_forecast']) / $grandTotal['annual_budget']) * 100;
                }
        ...
        case 'upload_certificate':
            ...
            try {
                ...
                // Copy data from budget_data to certificates table with cluster consideration
                $copyDataSql = "INSERT INTO certificates 
                    (year, category_name, period_name, budget, actual, forecast, actual_plus_forecast, 
                     variance_percentage, quarter_number, start_date, end_date, certificate_path, uploaded_by)";
                
                // Add cluster column if it exists in certificates table
                $checkColumn = "SHOW COLUMNS FROM certificates LIKE 'cluster'";
                $columnResult = $conn->query($checkColumn);
                $hasClusterColumn = $columnResult->num_rows > 0;
                
                if ($hasClusterColumn) {
                    $copyDataSql .= ", cluster ";
                }
                
                $copyDataSql .= "SELECT year, category_name, period_name, budget, actual, forecast, actual_plus_forecast,
                           variance_percentage, quarter_number, start_date, end_date, ?, 'admin'";
                
                if ($hasClusterColumn) {
                    $copyDataSql .= ", cluster ";
                }
                
                $copyDataSql .= "FROM budget_data 
                    WHERE year2 = ? ";
                
                // Add cluster condition if user has a cluster
                if ($userCluster) {
                    $copyDataSql .= " AND cluster = ? ";
                }
                
                $copyStmt = $conn->prepare($copyDataSql);
                
                // Bind parameters based on whether user has a cluster and certificates table has cluster column
                if ($hasClusterColumn && $userCluster) {
                    $copyStmt->bind_param("sis", $filePath, $selectedYear, $userCluster);
                } else if ($userCluster) {
                    $copyStmt->bind_param("sis", $filePath, $selectedYear, $userCluster);
                } else {
                    $copyStmt->bind_param("si", $filePath, $selectedYear);
                }
                
                if (!$copyStmt->execute()) {
                    throw new Exception('Failed to copy budget data to certificates table: ' . $copyStmt->error);
                }
                
                $affectedRows = $copyStmt->affected_rows;
                
                // Update budget_data table to mark as certified with cluster consideration
                $updateBudgetSql = "UPDATE budget_data SET certified = 'certified' WHERE year2 = ? ";
                
                // Add cluster condition if user has a cluster
                if ($userCluster) {
                    $updateBudgetSql .= " AND cluster = ? ";
                    $updateStmt = $conn->prepare($updateBudgetSql);
                    $updateStmt->bind_param("is", $selectedYear, $userCluster);
                } else {
                    $updateStmt = $conn->prepare($updateBudgetSql);
                    $updateStmt->bind_param("i", $selectedYear);
                }
                
                if (!$updateStmt->execute()) {
                    throw new Exception('Failed to update budget_data certified status: ' . $updateStmt->error);
                }
                ...
                echo json_encode([
                    ...
                    'records_certified' => $affectedRows,
                    'filename' => $fileName
                ]);
                exit;
